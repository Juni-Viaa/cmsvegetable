                ...
                @forelse($data as $index => $row)
                    ...
                    <div
                        ...
                        <!-- Kolom: Preview Gambar -->
                            <img src="{{ Storage::url($row->image_path) }}" alt="" class="hidden md:flex rounded-2xl object-cover w-[90px] h-[90px]">
                        <!-- Kolom: Judul Blog -->
                            <div class="flex flex-col w-4/12 pl-4 border-gray-300">
                                <h3 class="text-indigo-950 text-xl font-bold break-words">{{ $row->title }}</h3>
                            </div>
                        </div>

                        <!-- Kolom: Type -->
                        <div  class="hidden md:flex flex-col text-center w-4/12">
                        ...
                        <div class="hidden md:flex flex-row items-center gap-x-3">
                            @include('components.modal_edit', [
                                ...
                                'form_action' => route('admin.blogs.update', ':id'),
                                'form_method' => 'PUT',
                                'id_field' => 'blog_id',
                                'fields' => $editFields,
                                'data' => $data,
                            ...
                            @include('components.modal_delete', [
                                'id_field' => 'blog_id',
                                'delete_route' => route('admin.blogs.destroy', ':id'),
                            ])
                        </div>
                    </div>
                    @empty
