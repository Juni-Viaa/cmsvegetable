                ...
                <div class="item-card flex flex-row justify-between items-center">
                    ...
                    <div class="hidden md:flex flex-row items-center gap-x-3">
                        @include('components.modal_edit', [
                                ...
                                'fields' => $editFields,
                                'data' => $data,
                                'edit_route' => 'admin.principles.getPrinciple',
                            ])
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.edit-btn').forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            const form = modal.querySelector('form');
             ...
             // Debug
            console.log('Principle ID:', principleId);
            console.log('Modal ID:', modalId);
            
            // Update form
            if (form.getAttribute('action').includes(':id')) {
                const actionUrl = form.getAttribute('action').replace(':id', principleId);
                form.setAttribute('action', actionUrl);
            
            // Ambil data 
            fetch({{ url('admin.principles') }}/${principleId}, {
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const subtitleInput = modal.querySelector('input[name="subtitle"]');
                    
                    if (nameInput) headingInput.value = admin.principles.name || '';
                    if (subtitleInput) subtitleInput.value = admin.principles.subtitle || '';
                    
                    // Tampilkan preview gambar jika ada
                    const imagePreview = modal.querySelector('#image-preview');
                    if (admin.thumbnail.image_path && imagePreview) {
                        imagePreview.src = `/storage/${principles.image_path}`;
                        imagePreview.style.display = 'block';
                    }

                    const imagePreview = modal.querySelector('#image-preview');
                    if (admin.icon.image_path && imagePreview) {
                        imagePreview.src = `/storage/${principles.image_path}`;
                        imagePreview.style.display = 'block';
                    }
                } else {
                    console.error('Failed to fetch Principles data:', data);
                }
            })
            .catch(error => {
    ...
    document.querySelectorAll('.delete-btn').forEach(function(button) {
        button.addEventListener('click', function() {
            const principleId = this.getAttribute('data-id');
            const principleName = this.getAttribute('data-name');
            
            if (confirm(`Apakah Anda yakin ingin menghapus Principle "${principleName}"?`)) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = `{{ url('admin.principles') }}/${principleId}`;
