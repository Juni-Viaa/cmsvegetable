...
<div class="p-4 sm:ml-64">
    <div class="p-4 rounded-lg mt-14">
        ...
        @include('components.modal_add', [
            ...
            'form_action' => route('gallery.store'),
            'form_method' => 'POST',
            'fields' => $addFields
        ])
        ...
        @include('components.table_admin', [
            ...
            'form_action' => route('gallery.update', ':id'),
            'form_method' => 'PUT',
            'id_field' => 'gallery_id',
            'fields' => $editFields,
            'data' => $data,
            'columns' => $columns,
            'delete_route' => route('gallery.destroy', ':id'),
            'edit_route' => 'gallery.getGallery'
        ])
    </div>
</div>
    ...
    document.querySelectorAll('.delete-btn').forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            if (confirm(`Apakah Anda yakin ingin menghapus gallery "${galleryName}"?`)) {
                ...
                form.method = 'POST';
                form.action = `{{ url('gallery') }}/${galleryId}`;
