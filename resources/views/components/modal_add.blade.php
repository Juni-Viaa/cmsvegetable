...
<!-- Main modal -->
<div id="{{ $modal_id }}" data-modal-backdrop="static" tabindex="-1" aria-hidden="true" class="hidden fixed inset-0 z-50 overflow-y-auto bg-white/80 backdrop-blur-sm">
    <div class="flex items-center justify-center min-h-screen px-4 py-8">
        <div class="bg-white shadow-2xl border border-neutral-200" style="width: 480px;">
            ...
            <div class="flex items-center justify-between px-8 py-6 border-b border-neutral-100">
                ...
                <button data-modal-hide="{{ $modal_id }}" type="button" class="text-neutral-400 hover:text-neutral-700 rounded-full p-2 transition focus:outline-none focus:ring-2 focus:ring-neutral-300">
            ...
            <form action="{{ $form_action }}" method="{{ $form_method === 'GET' ? 'GET' : 'POST' }}" enctype="multipart/form-data" class="px-8 py-6">
                @csrf
                    ...
                    @foreach ($fields as $field)
                        <div class="col-span-2">
                            ...
                            @switch($field['type'])
                                ...
                                @case('number')
                                @case('password')
                                    <input type="{{ $field['type'] }}" name="{{ $field['name'] }}" id="{{ $field['name'] }}" placeholder="{{ $field['placeholder'] ?? '' }}" value="{{ old($field['name'], $field['value'] ?? '') }}" class="block w-full rounded-lg border border-neutral-200 bg-neutral-50 focus:bg-white shadow-none focus:border-neutral-400 focus:ring-2 focus:ring-neutral-200 text-sm text-neutral-900 placeholder-neutral-400 @error($field['name']) border-red-400 @enderror transition" {{ ($field['required'] ?? false) ? 'required' : '' }}>
                                @break
                                ...
                                @case('textarea')
                                    <textarea name="{{ $field['name'] }}" id="{{ $field['name'] }}" rows="{{ $field['rows'] ?? 4 }}" placeholder="{{ $field['placeholder'] ?? '' }}" class="block w-full rounded-lg border border-neutral-200 bg-neutral-50 focus:bg-white shadow-none focus:border-neutral-400 focus:ring-2 focus:ring-neutral-200 text-sm text-neutral-900 placeholder-neutral-400 @error($field['name']) border-red-400 @enderror transition" {{ ($field['required'] ?? false) ? 'required' : '' }}>{{ old($field['name'], $field['value'] ?? '') }}</textarea>
                                @break
                                ...
                                @case('checkbox')
                                    <div class="flex items-center">
                                        <input type="checkbox" name="{{ $field['name'] }}" id="{{ $field['name'] }}" value="{{ $field['value'] ?? '1' }}" {{ old($field['name'], $field['checked'] ?? false) ? 'checked' : '' }} class="rounded border-neutral-300 text-neutral-700 shadow-sm focus:ring-neutral-400">
                                    ...
                                    @if(isset($field['options']) && is_array($field['options']))
                                        @foreach ($field['options'] as $key => $option)
                                            <div class="flex items-center mb-2">
                                                <input type="radio" name="{{ $field['name'] }}" id="{{ $field['name'] }}_{{ $key }}" value="{{ is_array($option) ? $option['value'] : $key }}"{{ old($field['name'], $field['value'] ?? '') == (is_array($option) ? $option['value'] : $key) ? 'checked' : '' }} class="rounded border-neutral-300 text-neutral-700 shadow-sm focus:ring-neutral-400">
