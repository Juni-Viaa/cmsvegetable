...
@section('content')
    ...
    <div class="relative h-64 flex items-center justify-center bg-fixed bg-center bg-cover"
        style="background-image: url('{{ asset('images/parallax banner.jpg') }}');">
    ...
    <div x-data="galleryPage()" class="max-w-7xl mx-auto px-4 mt-8 font-poppins">
        ...
        <div x-data="{ open: true }"
            class="w-full mb-10 bg-gradient-to-br from-green-100 to-green-50 p-6 rounded-2xl shadow-md">
            <div class="flex justify-between items-center cursor-pointer" @click="open = !open">
                <h2 class="text-2xl font-bold text-black flex items-center gap-2 ml-1">
                    <svg xmlns="http://www.w3.org/2000/svg" class="size-6 text-black fill-current" viewBox="0 0 24 24"
                        fill="currentColor">
                        <path
                ...
                <div class="flex flex-col lg:flex-row gap-6">
                    <div
                        ...
                        <div class="flex flex-col gap-3">
                            <label
                                class="inline-flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-2 cursor-pointer hover:border-green-400 transition-all duration-200">
                                ...
                                <span class="text-sm font-medium text-gray-700 group-hover:text-green-700 transition">Latest
                                    Update</span>
                            </label>
                            <label
                                class="inline-flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-2 cursor-pointer hover:border-green-400 transition-all duration-200">
                                <input type="radio" name="sort" value="az"
                                    ...
                                    class="accent-green-600 focus:ring-2 focus:ring-green-400 focus:outline-none">
                                <span class="text-sm font-medium text-gray-700 group-hover:text-green-700 transition">Sort
                                    A–Z</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div
                        class="bg-white p-4 rounded-2xl shadow-sm border border-green-200 hover:shadow-md transition w-full lg:w-2/3">
                            ...
                            @foreach ($categories as $cat)
                                <label
                                    ...
                                    <input type="checkbox" name="category[]" value="{{ $cat->category_name }}"
                                        ...
                                        class="accent-green-600 focus:ring-2 focus:ring-green-400 focus:outline-none">
                                    <span
                                        class="text-sm font-medium text-gray-700">{{ ucfirst($cat->category_name) }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
        ...
        <div class="flex justify-end mb-6">
            <button id="toggleView"
                class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-full shadow-md flex items-center gap-2 transition">
        ...
        <div id="blogContainer"
            class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-6 transition-all duration-300">
        ...
        {{-- Modal --}}
        <div x-show="modalOpen" x-transition @click.self="closeModal" x-cloak
            class="fixed inset-0 z-50 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center">
