        ...
        <div class="max-w-6xl mx-auto space-y-6">
            ...
            <div x-data="{ images: ['{{ asset('images/sayur-kol-banner.jpg') }}', '{{ asset('images/wortel.jpg') }}', '{{ asset('images/brokoli.jpg') }}', '{{ asset('images/tomat.jpg') }}'], activeIndex: 0, init() { setInterval(() => { this.activeIndex = (this.activeIndex + 1) % this.images.length; }, 4000); } }" class="relative h-64 rounded-xl overflow-hidden shadow-xl bg-black group">
                <template x-for="(image, index) in images" :key="index">
                    <img x-show="activeIndex === index" x-transition:enter="transition-opacity duration-1000"
                        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" :src="image"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <div class="lg:col-span-2 bg-white shadow-xl rounded-xl p-6 fade-in-up">
                    ...
                    @forelse($comments as $comment)
                        <div class="bg-gray-100 p-4 rounded-lg" x-data="{ showReply: false, replyContent: '' }">
                            <div class="flex gap-4 items-start">
                                <div
                                    class="w-10 h-10 rounded-full bg-yellow-400 flex items-center justify-center text-white font-bold">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div x-data="{ showReply: false, replyContent: '' }">
                                        ...
                                        <form x-show="showReply" x-cloak x-data x-ref="replyForm" method="POST"
                                            ...
                                            <textarea x-model="replyContent" name="content" rows="2" class="w-full p-2 border rounded mt-1"
                                    ...
                                    @if ($comment->replies && $comment->replies->count())
                                        <div class="mt-3 space-y-2 border-l border-gray-100">
                                            @foreach ($comment->replies as $reply)
                                                <div class="flex pl-4 bg-gray-50 gap-4 items-start items-center rounded-lg">
                                                    <div
                                                        class="w-10 h-10 rounded-full bg-yellow-400 flex items-center justify-center text-white font-bold">
                                                        {{ strtoupper(substr($comment->user->username ?? 'G', 0, 1)) }}
                                                    </div>
                    ...
                    <form id="commentForm" method="POST"
                        ...
                        @csrf
                        <textarea id="commentContent" name="content" class="w-full p-2 border rounded" placeholder="Tulis komentar anda..."
                            required></textarea>
            ...
            <div class="space-y-4 fade-in-up">
                ...
                @foreach ($related as $index => $item)
                    <div x-data="{ show: false }" x-init="setTimeout(() => show = true, {{ 200 * $index }})" x-show="show"
                        ...
                        role="button" tabindex="0" aria-label="Artikel terkait: {{ $item->title }}">
                        <div
                            class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 transition duration-300 rounded-xl z-10">
                        </div>
                        <div class="flex gap-4 items-center relative z-20">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-700 mt-1">{{ \Str::limit(strip_tags($item->content), 50) }}
                                </p>
                            </div>
                        </div>
                    </div>
